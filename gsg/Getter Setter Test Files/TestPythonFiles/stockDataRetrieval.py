...
def newStockDatabase(conn, stockTable, stock):
    ...
    cur = conn.cursor()
    try:
        createTableQ = "create table "+stock.code+" (date varchar, hour integer, recent_price float, recent_offer float, recent_bid float, stock_volume double);"
...
def inputNewStock():
    ...
    while badInput==True:
        try:
            ...
            if raw_input("Incorrect input. Note: ASX code cannot be more than 3 chars. Press 'x' to exit or anything else to try again")=='x':
                return False
        badInput = True
...
def main():
    programEnd = False;
    dbLocation = os.path.dirname(sys.path[0])
